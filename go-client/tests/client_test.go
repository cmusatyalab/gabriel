...
const (
	launchResponseWait     = 2 * time.Second
	targetEngineSwitchWait = 2 * time.Second
...
func repeatingProducer(payload string, interval time.Duration) client.Producer {
...
func TestEndToEnd(t *testing.T) {
	...
	grpcClient, _ := client.NewGrpcClient(grpcServerAddr, []*client.InputProducer{producer}, consumer)
...
func TestMultipleEngines(t *testing.T) {
	...
	producer := client.NewInputProducer(
		"producer-1", repeatingProducer("hi", inputInterval), []string{"0", engine1, engine2},
	)
	counts := &engineCounts{}
	grpcClient, _ := client.NewGrpcClient(grpcServerAddr, []*client.InputProducer{producer}, counts.consumer)
...
func TestInvalidEngineTarget(t *testing.T) {
	...
	consumer := func(result *gabrielpb.Result) {}
	grpcClient, err := client.NewGrpcClient(grpcServerAddr, []*client.InputProducer{producer}, consumer)
	if err != nil {
...
func TestEmptyInputFrame(t *testing.T) {
	...
	producer := client.NewInputProducer("producer-1", emptyOnce, []string{"0"})
	var receivedResponse atomic.Bool
	consumer := func(result *gabrielpb.Result) {
		receivedResponse.Store(true)
	}
