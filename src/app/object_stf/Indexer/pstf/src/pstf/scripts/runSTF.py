...
# 
# Code ownership:
# First copied from Zhuo Chen's indexer_pool.py for Gigasight work.
# Wenlu Hu modified for use in Gabirel 
# 
# !!!!!!
# Note: This python file is not for running in the gabriel folder. It 
# must be placed in the STF package folder Indexer/pstf/src/pstf/scripts
# to run. The gabriel proxy.py will start this program through things 
# like os.system().
#
...
#    print [(CLASSES[class_index], "%.02f" % score) for class_index, score in enumerate(ILP) if score > 0.7]
    print [CLASSES[class_index] for class_index, score in enumerate(ILP) if score > 0.9]
    
    #TODO return the most likely result
...
#    print processes
    ...
    # Image is ready in file ${name}${process_num}.${pic_suffix}
    # TODO parallel: pool.map
