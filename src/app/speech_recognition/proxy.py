...
class SpeechThread(AppProxyThread):
    def __init__(self, app_addr, image_queue, output_queue):
        super(SpeechThread, self).__init__(image_queue, output_queue)
...
if __name__ == "__main__":
    APP_PATH = "java -jar ./SpeechRecognitionServer.jar"
    APP_PORT = 9092

    app_thread = AppLauncher(APP_PATH, APP_PORT, is_print=False)
    app_thread.start()
    ...
    app_thread = SpeechThread(app_addr, image_queue, output_queue)
    app_thread.start()
    app_thread.isDaemon = True
