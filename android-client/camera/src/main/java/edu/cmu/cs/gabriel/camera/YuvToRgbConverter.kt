// Based on: https://github.com/android/camera-samples/blob/4aac9c7763c285d387194a558416a4458f29e275/CameraUtils/lib/src/main/java/com/example/android/camera/utils/YuvToRgbConverter.kt
/*
...
class YuvToRgbConverter(context: Context) {
    ...
    companion object {
        fun imageToByteArray(image: ImageProxy, outputBuffer: ByteArray, pixelCount: Int) {
            // TODO: profile a check that image.format != ImageFormat.YUV_420_888
            ...
            imagePlanes.forEachIndexed { planeIndex, plane ->
                ...
                // Only the Y plane has a value for every pixel, U and V have half the resolution
                // i.e.
                //
                ...
                val rowLength = if (pixelStride == 1 && outputStride == 1) {
                    ...
                    // Take into account that the stride may include data from pixels other than
                    // this particular plane and row, and that could be between pixels and not after
                    // every pixel:
                    //
                    // |---- Pixel stride ----|                    Row ends here --> |
                    // | Pixel 1 | Other Data | Pixel 2 | Other Data | ... | Pixel N |
                    //
                    // We need to get (N-1) * (pixel stride bytes) per row + 1 byte for the last
                    // pixel
                ...
                for (row in 0 until planeHeight) {
                    ...
                    val newPosition = (((row + planeCrop.top) * rowStride) +
                            (planeCrop.left * pixelStride))
                    planeBuffer.position(newPosition)

                    if (pixelStride == 1 && outputStride == 1) {
                        // When there is a single stride value for pixel and output, we can just
                        // copy the entire row in a single step
