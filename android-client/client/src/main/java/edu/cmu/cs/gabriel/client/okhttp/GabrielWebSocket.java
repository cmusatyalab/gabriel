// Based on https://github.com/square/okhttp/blob/parent-3.11.0/okhttp/src/main/java/okhttp3/internal/ws/RealWebSocket.java
// I had to copy this file in order to call setTcpNoDelay(true) on the underlying socket.
/*
...
import static edu.cmu.cs.gabriel.client.okhttp.WebSocketProtocol.CLOSE_CLIENT_GOING_AWAY;
import static edu.cmu.cs.gabriel.client.okhttp.WebSocketProtocol.CLOSE_MESSAGE_MAX;
...
public final class GabrielWebSocket implements WebSocket, WebSocketReader.FrameCallback {
    ...
    public GabrielWebSocket(Request request, WebSocketListener listener, Random random,
    ...
    public void connect(OkHttpClient client) {
        ...
        call.enqueue(new Callback() {
            @Override public void onResponse(Call call, Response response) {
                ...
                try {
                    listener.onOpen(GabrielWebSocket.this, response);
                    String name = "OkHttp WebSocket " + request.url().redact();
                    initReaderAndWriter(name, streams);
                    streamAllocation.connection().socket().setSoTimeout(0);
                    streamAllocation.connection().socket().setTcpNoDelay(true);
                    loopReader();
                } catch (Exception e) {
                    failWebSocket(e, null);
                }
    ...
    boolean writeOneFrame() throws IOException {
        ...
        synchronized (GabrielWebSocket.this) {
            if (failed) {
